# ABOUTME: Schedule and Game classes for parsing an NCAAB team's game schedule.
# ABOUTME: Vendored from sportsipy; all transport removed, parsing only.
import pandas as pd
import re
from pipeline.scraper.decorators import int_property_decorator
...
from pipeline.scraper.constants import (WIN,
                                        LOSS,
                                        HOME,
                                        AWAY,
                                        NEUTRAL,
...
from pipeline.scraper.ncaab.boxscore import Boxscore
        ...
        if not year:
            ...
            if not utils._url_exists(SCHEDULE_URL % (abbreviation.lower(),
               ...
               utils._url_exists(SCHEDULE_URL % (abbreviation.lower(),
                ...
                year = str(int(year) - 1)
        # utils.check_limit()
        doc = pq(utils.request_through_api_gateway(SCHEDULE_URL % (abbreviation.lower(), year)))
